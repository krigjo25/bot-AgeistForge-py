...
class MemberModeration(commands.Cog):
    ...
    member = SlashCommandGroup(name = "member", 
                               description = "Member mananger", 
    ...
    async def warn(self, 
        ...
        try:
            ...
            action = mod_utils.fetch_function_name(self.warn)                                       #   type: ignore

            await mod_utils.create_log_entry(ctx, reason, member=member, function_name=action)      #   type: ignore
